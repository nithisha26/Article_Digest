...
const Demo = () => {
  ...
  const [summaryLength, setSummaryLength] = useState(3); // Track the summary length (1, 2, or 3)
  ...
  // Function to read the article summary aloud
  const readAloud = () => {
    if (article.summary) {
      const speech = new SpeechSynthesisUtterance(article.summary);
      window.speechSynthesis.speak(speech);
      setIsSpeaking(true); // Set speaking state to true
      speech.onend = () => setIsSpeaking(false); // Set speaking state to false after speech ends
    }
  };
  ...
  const stopSpeech = () => {
    ...
    setIsSpeaking(false); // Set speaking state to false when speech is stopped
  };
